...
注意：这并不需要99%的访问都是热数据，只要90%的访问都是常规数据即可。而90%的访问都是常规数据，这是典型的互联网应用的规律。所以，基本上，绝大部分的应用的P99都可以达到1ms以下。这就和Redis的性能几乎一致，也就能让RedRock既扩大了数据到磁盘（因此可以有千倍的提升），同时也具有Redis的高性能。
